# Specification: Subcategory Selector with "Газовые" Disclaimer

## Overview
Adds a subcategory selector to the catalog page that appears after selecting a category. When "Газовые" subcategory is selected, displays a disclaimer message about chimney inclusion in pricing.
...
## Data Model Notes
- Items have `subcategory` field as optional string (convex/schema.ts:31)
- Subcategories are stored in `subcategorys` table with `name` field
- Existing query `dashboard.show_subcategories_by_category` can fetch subcategories by parent category
- No database schema changes required

## Dependencies
- Requires `dashboard.show_subcategories_by_category` query (already exists)
- Uses existing item.subcategory field
- No new database tables or indexes required

## UI/UX Considerations
- Subcategory selector should be visually grouped with category selector
- Message should be visually distinct but not overwhelming
- Consider using info/warning color scheme for the disclaimer message
- Message should be dismissible or persistent based on business requirements

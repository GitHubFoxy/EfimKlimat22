...
## Purpose
Define requirements for image management in Manager UI item forms, including upload, reordering, deletion, and preview functionality. Images are critical for product presentation and managers need full control over their order and content.
...
- References `manager-crud` for item creation and editing flows
- Leverages `components/manager/ImageField` component
- Depends on Convex storage API for upload and URL generation

...
Convex Auth provides an API and helper functions for implementing your middleware:
    ...
    createRouteMatcher is a helper function that uses the same syntax

as the middleware config. You call it with a list of glob patterns, and it returns a function that given the NextRequest returns whether the route matches.
...
To preload or load data on your Next.js server from your Convex backend, you can use preloadQuery and fetchQuery
...
You can call Convex mutations
and actions from Next.js Server Actions and POST or PUT Route Handlers
...
Security notice: ConvexAuthNextjsServerProvider uses cookies to store authentication state. Therefore to prevent CSRF attacks

you must not perform any side-effects from the Next.js server on GET requests. This means that only Convex queries are safe to call from Server Components and GET Route Handlers.

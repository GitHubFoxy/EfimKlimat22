...
export const getMappings = query({
  ...
  handler: async (ctx) => {
    ...
    for (const c of categories) if (c.legacyId) categoriesMap[c.legacyId] = c._id;
...
export const getPrefixes = query({
  ...
  handler: async (ctx) => {
    const tables = [
        "users", "brands", "items", "categories", "carts", "cartItems", "orders", "orderItems", "leads", "reviews"
    ] as const;
...
export const createDummyData = mutation({
  ...
  handler: async (ctx) => {
    ...
    await ctx.db.insert("brands", { name: "test", slug: "test", status: "active" });
    await ctx.db.insert("categories", { name: "test", slug: "test", level: 1, order: 1, isVisible: true });

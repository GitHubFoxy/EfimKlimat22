...
export const diagnose_category_items = query({
  ...
  handler: async (ctx, { categorySlug }) => {
    ...
    const subcategories = await ctx.db
      .query("categories")
      .withIndex("by_parent_order", (q) => q.eq("parentId", topLevelCategory._id))
      .collect();
    ...
    console.log(`Items directly assigned to ${topLevelCategory.name} (${topLevelCategory._id}):`, directItems.length);
    if (directItems.length > 0) {
      console.log("Sample:", directItems.map(i => ({ name: i.name, categoryId: i.categoryId })));
    }

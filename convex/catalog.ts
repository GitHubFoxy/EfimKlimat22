...
export const catalog_query_based_on_category_and_filter = query({
  ...
  handler: async (
    ctx,
    { category, filter, brand: brandParam, priceSort, cursor = 0 },
  ) => {
    // Input validation
    validateCursor(cursor)
    if (brandParam) validateStringLength(brandParam, 255, 'Brand')

    // Resolve brand param (slug or ID) to brandId
    let brandId: Id<'brands'> | undefined
    if (brandParam) {
      const resolved = await brand_resolve_from_url_param_internal(
        ctx,
        brandParam,
      )
    ...
    if (priceSort === 'asc') {
      ...
      allItems.sort(
        (a, b) =>
          (b.ordersCount ?? 0) - (a.ordersCount ?? 0) ||

...
export function ItemFormDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      {/* Make dialog wider and limit height; inner area will scroll */}
      <DialogContent className='w-full max-w-[95vw] h-[90vh] overflow-hidden'>
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Edit Item' : 'Add New Item'}</DialogTitle>
        </DialogHeader>

        {/* Form: header and footer stay fixed, fields scroll */}
        <form onSubmit={handleSubmit} className='flex flex-col h-full'>
          <div className='overflow-y-auto space-y-4 pr-2 max-h-[calc(90vh-140px)]'>
                  ...
                  <div className='rounded-md border bg-muted/5 p-3 min-h-[56px] text-sm text-gray-700'>
                    {formData.description ? (
                      <div className='whitespace-pre-wrap'>
                        {truncate(formData.description, 250)}
                      </div>
                    ) : (

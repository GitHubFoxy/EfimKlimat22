...
function isImageFile(fileName) {
  ...
  return ['.jpg', '.jpeg', '.png', '.webp', '.gif', '.tiff', '.bmp'].includes(
    ext,
  )
}
...
async function toWebpBuffer(filePath) {
  ...
  return sharp(input).webp({ quality: 80, effort: 6 }).toBuffer()
}
...
async function seed() {
  ...
  for (const item of items) {
    ...
    if (
      !imagesDirRelRaw ||
      typeof imagesDirRelRaw !== 'string' ||
      imagesDirRelRaw.trim() === ''
    ) {
      console.warn(
        `No images path provided for ${item.name}, inserting without images`,
      )
    } else {
      ...
      try {
        ...
        console.warn(
          `Images directory not found at ${imagesDirAbs} for ${item.name}, inserting without images`,
        )
      }
      if (!files.length) {
        console.warn(
          `No images found at ${imagesDirAbs} for ${item.name}, inserting without images`,
        )
      }
    }
    ...
    const subcategoriestr = item.subcategory

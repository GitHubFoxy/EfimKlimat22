...
- [ ] Block privileged access for users with `status: blocked` in shared auth helpers.
- [ ] Harden checkout/order creation on server:
  - [ ] Server-owned delivery pricing (ignore client delivery price input).
  - [ ] Enforce orderability: item exists, status allows ordering, in stock, valid quantity.
  - [ ] Prevent duplicate orders from concurrent checkout submits (idempotency/locking).
  - [ ] Make public order number generation collision-safe.
- [ ] Sanitize public order response (`orders.get_order_by_id`) to avoid leaking internal fields/PII.
- [ ] Disable or remove production-exposed debug/test surfaces:
  - [ ] `convex/test_category_filter.ts`
...
- [ ] Tighten cart integrity rules:
  - [ ] Enforce quantity bounds in merge/update flows (integer + max cap).
  - [ ] Validate `itemId` existence/orderability in `addItem`.
